...
# Keys that must never appear in tool outputs (shared with worker.py)
SENSITIVE_KEYS = frozenset({
    "vendor_secret", "unit_cost_usd", "ACME-TIER-2-REBATE",
    "BETA-PRICE-FLOOR", "credential", "token", "password",
    "debug", "candidate_note", "user_permissions",
})
...
class ToolRegistry:
    ...
    def call(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        user_permissions = args.get("user_permissions", [])
        required_perm = tool_permissions.get(name)
        if required_perm and required_perm not in user_permissions:
            raise PermissionError(f"用户缺少执行工具 {name} 所需的权限: {required_perm}")
        ...
        for _ in range(max_attempts):
            ...
            try:
                ...
                if isinstance(result, dict):
                    return {k: v for k, v in result.items() if k not in SENSITIVE_KEYS}
                return result
